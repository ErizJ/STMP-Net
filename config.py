...
_C.DATA.TEXTURE_NUM_CLASSES = 11
_C.DATA.STRUCTURE_NUM_CLASSES = 6
...
_C.MODEL = CN()
# Model type
_C.MODEL.TYPE = "stmp_net"
_C.MODEL.BACKBONE = "ViT-B-16"
# Model name
_C.MODEL.NAME = "STMP-Net"
...
_C.num_scene = 9
_C.num_dist = 11
_C.num_texture = 11     # 纹理失真类别数
_C.num_structure = 6    # 结构失真类别数
_C.visual = False
_C.scene = False
_C.dist = False
_C.texture = False      # 纹理分支开关
_C.structure = False    # 结构分支开关
_C.visual_only = False  # 纯视觉模式：不使用任何文本提示分支
_C.visual_only_num_queries = 20  # 纯视觉模式下的可学习query token数
_C.data_percent = 0.8
_C.print = False
_C.GAMMA = 1.0          # smoothl1_loss 权重
_C.DELTA = 1.0          # fidelity_loss 权重（排序一致性损失）
_C.single_scale_window = False  # 消融实验：使用单尺度窗口（禁用双尺度设计）
...
def _update_config_from_file(config, cfg_file):
    ...
    with open(cfg_file, "r", encoding="utf-8") as f:
        cfg_str = f.read()
    config.merge_from_other_cfg(config.load_cfg(cfg_str))
    config.freeze()

...
from datasets import IQA_build_loader
from config import get_config
from logger import create_logger
from lr_scheduler import build_stage1_scheduler, WarmupMultiStepLR
from stmp_net import STMPNet, load_clip_to_cpu
from datasets import CLIP_IQA_build_loader1, CLIP_IQA_build_loader2
...
def parse_option():
    parser = argparse.ArgumentParser(
        "STMP-Net training and evaluation script", add_help=False
    )
    parser.add_argument(
        "--cfg",
        type=str,
    ...
    parser.add_argument("--gamma", type=float, help="smoothl1_loss weight")
    parser.add_argument("--delta", type=float, help="fidelity_loss weight")
    parser.add_argument("--visual", action="store_true")
    parser.add_argument("--data_percent", type=float)
    parser.add_argument("--print", action="store_true")
    # 单卡训练：local_rank 默认为 0
...
if __name__ == "__main__":
    ...
    for i in range(rnum):
        ...
        cudnn.benchmark = False  # Windows+PyTorch1.12下benchmark=True会导致严重内存碎片化OOM
        ...
        # 预热CUDA allocator，避免Windows上PyTorch 1.12的内存碎片化OOM
        _warmup = torch.zeros(1, 3, 224, 224, device='cuda')
        del _warmup
